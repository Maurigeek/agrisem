...
function seedDatabase() {
    ...
    // ====================
    const cultures = [
        { id: db_1.db.getNextCultureId(), name: 'Maïs', code: 'MAIS' },
        { id: db_1.db.getNextCultureId(), name: 'Riz', code: 'RIZ' },
        { id: db_1.db.getNextCultureId(), name: 'Tomate', code: 'TOMATE' },
        { id: db_1.db.getNextCultureId(), name: 'Oignon', code: 'OIGNON' },
        { id: db_1.db.getNextCultureId(), name: 'Gombo', code: 'GOMBO' },
        { id: db_1.db.getNextCultureId(), name: 'Piment', code: 'PIMENT' },
    ];
    db_1.db.cultures = cultures;
    // ====================
    // VARIETIES
    // ====================
    const varieties = [
    ...
    varieties.forEach((variety, vIndex) => {
        ...
        for (let i = 0; i < productsPerVariety; i++) {
            const productId = db_1.db.getNextProductId();
    ...
    console.log(`   - ${db_1.db.users.length} users`);
    console.log(`   - ${db_1.db.cultures.length} cultures`);
    console.log(`   - ${db_1.db.varieties.length} varieties`);
    console.log(`   - ${db_1.db.products.length} products`);
    console.log(`   - ${db_1.db.articles.length} articles`);
    console.log(`   - ${db_1.db.orders.length} orders`);
    console.log(`   - ${db_1.db.threads.length} threads`);
    console.log(`   - ${db_1.db.messages.length} messages`);
}
